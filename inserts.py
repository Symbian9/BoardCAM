...
def gen(left_start_cx, spacing, setback):
    content = ""
...
def gen_inserts2(params, inserts_number, spacing, offset):
    """
    从竖中线向左右两侧生成嵌件位置
    :param params:
    :param inserts_number: 单侧单排嵌件个数
    ...
    if inserts_number % 2 == 1:
        ...
        for i in range(int((inserts_number - 1) / 2)+1):
            content += gen(left_mid_line + spacing * i, spacing, stand_setback)
            content += gen(left_mid_line - spacing * i, spacing, stand_setback)

        # 右侧
        right_mid_line = vertical_mid_line + stand_width / 2
        for i in range(int((inserts_number - 1) / 2) + 1):
            content += gen(right_mid_line + spacing * i, spacing, stand_setback)
            content += gen(right_mid_line - spacing * i, spacing, stand_setback)
        ...
        for i in range(1, int(inserts_number/2)+1):
            if i == 1:
                content += gen(left_mid_line + spacing/2, spacing, stand_setback)
            else:
                content += gen(left_mid_line + spacing/2+spacing * (i-1), spacing, stand_setback)
        ...
        for i in range(1, int(inserts_number / 2) + 1):
            if i == 1:
                ...
                content += gen(left_mid_line - spacing * (i-1)-spacing / 2, spacing, stand_setback)
        ...
        for i in range(1, int(inserts_number / 2) + 1):
            if i == 1:
                content += gen(right_mid_line + spacing / 2, spacing, stand_setback)
            else:
                content += gen(right_mid_line + spacing / 2 + spacing * (i - 1), spacing, stand_setback)
        ...
        for i in range(1, int(inserts_number / 2) + 1):
            if i == 1:
                ...
                content += gen(right_mid_line - spacing * (i - 1) - spacing / 2, spacing, stand_setback)
